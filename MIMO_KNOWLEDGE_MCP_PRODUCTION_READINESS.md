...
| GitHub token read-only, contents-only, scoped to the 13 repos | **manual action** — create the fine-grained PAT this way; the server only ever calls read endpoints (`commits`, `git/trees`, `git/blobs`) |
...
|---|---|---|
| `mimo.knowledge.github.token` | fine-grained PAT, read-only Contents on the 13 configured repos | rotate in GitHub → update the properties file → `systemctl restart mcp` (cache rebuilds on start) |
...
3. Add the `MIMO_MCP_RELOAD_TOKEN` Actions secret in each of the 13 service
